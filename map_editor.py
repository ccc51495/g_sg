...
yellow = (255,255,9)#mountain
red = (255,0,0)#impossible mountain
ice = (50,200,180)#ice
...
class Map_information():
    def __init__(self):
        self.landform = 0 #0 平原 1 森林 2 河流 3 沙漠 4 高山 5 雪地 6 丘陵 7 山脉（不可通行）
        ...
        # 1 农夫 2 轻步兵 3 重步兵 4 近卫军 5 青州军 6 藤甲军 7 蛮族军 8 黄巾军 9 山越勇士
...
def map_size(he,wi):
    ...
    i = a.add_new((940,280), "image/m10.bmp","i")
    clear = a.add_new((940,400),"image/m5.bmp","clear")

    h0 = he
    w0 = wi
    maplist = [[Map_information() for i in range(he)] for j in range(wi)] 

    # maplist = [[Map_information for i in range(h)] for j in range(w)]
            ...
            if MyActiveD:
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if a.mode == 1:
                        ...
                        draw_impossiblemountain(screen,MyActiveD[0],MyActiveD[1])
                        maplist[MyActiveD[1]][MyActiveD[0]].landform = 7
...
def draw_mountain(screen,x,y):
    pygame.draw.rect(screen,yellow,([21+x*30,21+y*30],[29,29]))
...
def draw_impossiblemountain(screen,x,y):
...
def map_set_size():
    ...
    msg = '请输入长度和宽度'
    title = '地图大小设置'
    start = easygui.multenterbox(msg,title,fields,[10,10])
